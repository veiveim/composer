...
void WaveSequence::fast_fourier_transform(std::vector<int> samples, int nb_samples)
{
    nb_waves = nb_samples;
...
WaveExplorer::WaveExplorer() 
{
    ...
    nb_samples = 0;
    samples.resize(NB_SAMPLES_PER_FRAME);
    next_sample_pos = -1;
}
...
int64_t WaveExplorer::load_file(const char *file) 
{
...
void get_frame_samples(AVFrame *frame, int &next_sample_pos, std::vector<int> &samples, int &nb_samples)
{
    switch (frame->format) {
        case AV_SAMPLE_FMT_S16P:
            for (; next_sample_pos < frame->nb_samples && nb_samples < NB_SAMPLES_PER_FRAME; next_sample_pos++) {
                int16_t value = 0;
...
int WaveExplorer::get_next_frame_samples() 
{
    nb_samples = 0;
    while (nb_samples < NB_SAMPLES_PER_FRAME) {
...
const WaveSequence* WaveExplorer::get_next_frame_waves()
{
    if (get_next_frame_samples()) {
        return NULL;
    }
...
void WaveExplorer::draw_samples(cairo_t *cr, int width)
{
    for (int i = 0; i < width; i++) {
        ...
        */
        cairo_move_to(cr, i, 400);
        cairo_line_to(cr, i, 400 - ((int)samples[i] / 40));
    }
    cairo_stroke(cr);
}


void WaveExplorer::draw_kwaves(cairo_t *cr, int width, int k) 
{
    //std::cout << "draw_kwaves, nb_samples:" << nb_samples << ", nb_waves:" << waves.nb_waves << " kwaves:" << k << std::endl;
    ...
    for (int i = 0; i < width; i++) {
        ...
        */
        cairo_move_to(cr, i, 400 + value - 2);
        cairo_line_to(cr, i, 400 + value + 2);
    }
...
void WaveExplorer::draw_frames(const char *file_path, int k) 
{
    ...
    int width = (NB_SAMPLES_PER_FRAME > 30000) ? 30000 : NB_SAMPLES_PER_FRAME;

    cairo_surface_t *surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, width, 800);
